...
def _load_rag():
    global _rag_loaded
    global generate_grounded_answer, REGULAR_RAG_AVAILABLE
    ...
    chat_text = None  # vision model not needed for text queries
...
def answer_text(query: str) -> tuple[str, dict]:
    """Run a text query through safety → regular RAG. Returns (answer, meta)."""
    _load_rag()
    ...
    if not REGULAR_RAG_AVAILABLE:
        return ("⚠️ optimized_rag.py not found. Make sure it's in the same "
                "directory as gymbot_api.py."), {"system": "none"}

    answer = generate_grounded_answer(query)
    return answer, {"system": "regular"}
