...
# HELPERS
# ------------------------------
def chat_text(user_text: str, max_new_tokens=350, temperature=0.4):
    messages = [
    ...
    # 2) Convert to model inputs (a mapping with input_ids/attention_mask)
    inputs = processor(
        text=[prompt_text],
        images=None,            # add a list of PIL images later if needed
        return_tensors="pt"
    ).to(model.device)
